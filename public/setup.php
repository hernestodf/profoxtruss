...
$root = dirname(__DIR__); // public/ → raiz do projeto
$checks = [];
$errors = [];
...
<h2>3. Teste do Bootstrap (index.php)</h2>
<?php
$bootstrapOk = true;
$bootstrapErrors = [];
...
try {
    ob_start();
    $origDisplay = ini_get('display_errors');
    ini_set('display_errors', '1');
    
    // Simula bootstrap do public/index.php
    ...
    if (!class_exists('ErrorHandler')) {
        throw new Exception('ErrorHandler.php não encontrado em app/utils/');
    }
    if (!class_exists('Router')) {
        throw new Exception('Router.php não encontrado');
    }
    
    require ROOT . '/routes.php';
    
    // Checa arquivos críticos
    $criticalFiles = [
        'app/controllers/ApiController.php',
        'app/controllers/HomeController.php',
        'app/controllers/PecaController.php',
    ...
    foreach ($criticalFiles as $f) {
        if (!file_exists(ROOT . '/' . $f)) {
            $bootstrapErrors[] = ['err', "FALTA: $f — upload incompleto"];
            $bootstrapOk = false;
        }
    }
    
    $output = ob_get_clean();
    ini_set('display_errors', $origDisplay);
    
    if ($output !== '') {
        $bootstrapErrors[] = ['err', "Erro PHP: " . strip_tags($output)];
        $bootstrapOk = false;
    }
} catch (Throwable $e) {
    ob_end_clean();
    $bootstrapErrors[] = ['err', $e->getMessage() . ' (linha ' . $e->getLine() . ' em ' . basename($e->getFile()) . ')'];
    $bootstrapOk = false;
}

if ($bootstrapOk && empty($bootstrapErrors)) {
    echo '<div class="row"><span class="icon" style="color:#22c55e">✓</span> Bootstrap OK — sistema deve funcionar</div>';
} else {
    foreach ($bootstrapErrors as [$type, $msg]) {
        echo '<div class="row"><span style="color:#ef4444">' . htmlspecialchars($msg) . '</span></div>';
    }
}

// Testa login padrão
$pdo2 = getPDO();
$user = $pdo2->query("SELECT email, role FROM users LIMIT 1")->fetch();
$pdo2 = null;
if ($user) {
    echo '<div class="row"><span class="icon" style="color:#22c55e">✓</span> Usuário admin: ' . htmlspecialchars($user['email']) . ' (senha: admin123)</div>';
} else {
    echo '<div class="row"><span style="color:#ef4444">✗ Nenhum usuário no banco — seed pode ter falhado</span></div>';
}
?>

<?php if (!empty($errors)): ?>
<h2>4. Problemas a corrigir</h2>
